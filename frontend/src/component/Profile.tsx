...
const Profile: React.FC = () => {
    ...
    return (
      ...
      <div className="bg-gray-100 p-4 rounded-lg mb-6">
        <p className="text-lg"><strong>ID:</strong> {profileData.id}</p>
        <p className="text-lg"><strong>Email:</strong> {profileData.email}</p>
        <p className="text-lg"><strong>Name:</strong> {profileData.name}</p>
        <p className="text-lg"><strong>Credits:</strong> {profileData.credits}</p>
      </div>
              ...
              <tbody>
                {watchlist.map((item) => (
                  <tr key={item.id} className="border-t hover:bg-gray-100 transition">
                    <td className="p-3 font-medium text-gray-700">{item.stock.symbol}</td>
              ...
              <tbody>
                {transactions.map((transaction, index) => (
                  <tr key={index} className="border-t hover:bg-gray-100 transition">
                    ...
                    <td className="p-3 font-medium text-gray-700">{transaction.stock.symbol}</td>
                    <td className="p-3 capitalize">{transaction.type}</td>
                    <td className="p-3">{transaction.quantity}</td>
                    <td className="p-3 font-semibold">₹{transaction.price.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
